...
class Z04FundamentalSmokeTest(QCAlgorithm):
    """Export selected FineFundamental fields for a supplied ticker list."""

    _CHUNK = 2000

    def initialize(self):
        run_date_str = self.get_parameter("run_date") or datetime.utcnow().strftime("%Y%m%d")
        tickers_text = self.get_parameter("tickers") or "AAPL,MSFT"
        self._targets = {x.strip().upper() for x in tickers_text.split(",") if x.strip()}
        self._max_rows = int(self.get_parameter("max_rows") or 300)
        self._rows = {}
        ...
        self.log(f"Testing fundamentals for {len(self._targets)} target tickers on {run_date.date()}")
    ...
    def _coarse(self, coarse):
        ...
        for item in coarse:
            ...
            if ticker in self._targets and has_fundamental:
                selected.append(item.Symbol)
        return selected[: self._max_rows]
    ...
    def _fine(self, fine):
        for item in fine:
            ...
            if ticker not in self._targets:
                continue
            self._rows[ticker] = self._fundamental_row(item)
        return []

    def on_end_of_algorithm(self):
        self.set_runtime_statistic("META_target_count", str(len(self._targets)))
        ...
        rows = [",".join(header)]
        for ticker in sorted(self._targets):
            values = self._rows.get(ticker) or {"ticker": ticker}
    ...
    def _fundamental_row(self, item):
        ...
        balance = self._get_obj(financials, "balance_sheet", "BalanceSheet")
