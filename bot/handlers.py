...
class ShortBot:
    ...
    def format_message(self, df, mode='top', is_scheduled=False):
        ...
        if mode == 'top':
            ...
            title, metric_label, display_col, unit, sec_label = "Short Sale Volume (SOUT) 最新", "指标", df.columns[min(len(df.columns)-1, 2)], "", "详情"
    ...
    async def start(self):
        ...
        async def handle_sout(event): await event.respond(self.format_message(self.get_latest_data(FintelSout), mode='sout'))
